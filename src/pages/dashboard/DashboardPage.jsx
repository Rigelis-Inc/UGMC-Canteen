    ...
    <Layout>
      ...
      <div className="mb-7 animate-fadeIn">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm text-gray-500 mb-0.5">{greeting()}, {userProfile?.fullName?.split(" ")[0] || "there"}</p>
            <h1 className="text-[22px] font-bold text-gray-900 tracking-tight">Dashboard</h1>
          </div>
          <div className="flex items-center gap-2 text-[13px] text-gray-500 bg-white px-3.5 py-2 rounded-lg border border-gray-200 shadow-sm flex-shrink-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-white rounded-xl border border-gray-200 p-5 animate-pulse">
                ...
                <div className="h-7 bg-gray-100 rounded w-24 mb-1"></div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
            <div className="lg:col-span-2 bg-white rounded-lg border border-gray-200">
              <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Last 10 transactions this month</p>
                </div>
              </div>
              {recentMovements.length === 0 ? (
                <div className="p-10 text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-1">Start by receiving stock from suppliers</p>
                </div>
              ) : (
                <div className="divide-y divide-gray-50">
                  {recentMovements.map((movement) => {
                    ...
                    return (
                      <div key={movement.id} className="px-5 py-3 flex items-center gap-3 hover:bg-slate-50/70 transition-colors">
                        <div className={`p-1.5 rounded-md ${config.bg} flex-shrink-0`}>
                          <Icon size={14} className={config.text} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-gray-900 truncate">{movement.productName}</p>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] text-gray-400">{movement.storeName}</span>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-4">
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Stock Alerts</h3>
